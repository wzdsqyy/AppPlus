 ...
 */
class SimpleJavaJSWebChromeClient extends WebChromeClient {
...
//        result.cancel();
    ...
    public boolean onJsPrompt(WebView view, String url, String message, String defaultValue, JsPromptResult result) {
        ...
        if (!TextUtils.isEmpty(message)&&mSimpleJavaJsBridge.parseJsonFromJs(message)) {
            /*必须得有这行代码，否则会阻塞当前h5页面*/
            Log.d("Time", "onJsPrompt: "+(System.currentTimeMillis()-start));
            result.cancel();
            return true;
        }
        if (checkObjectNotNull(mWebChromeClient)) {
